...
function HotelListDummy() {
  const [hotels, setHotels] = useState(dummyHotels); // :흰색_확인_표시: 초기값을 바로 설정 (useEffect 불필요)
  const [loading, setLoading] = useState(false); // :흰색_확인_표시: 로딩 상태 관리
  const [searchParams, setSearchParams] = useSearchParams();
  const initialSort = searchParams.get("sort") || "recommend";
  const [activeSort, setActiveSort] = useState(initialSort);
  // 정렬 함수 (더미 데이터 기준으로 정렬)
  const handleSort = (sortType) => {
    let sortedHotels = [...dummyHotels]; // :흰색_확인_표시: 기존 상태(hotels)가 아닌 원본 데이터로 정렬
    switch (sortType) {
      case "recommend":
        sortedHotels = [...dummyHotels]; // :흰색_확인_표시: 원래 순서 유지
        break;
      case "price":
        sortedHotels.sort(
          (a, b) =>
    ...
    setHotels(sortedHotels); // :흰색_확인_표시: 정렬된 데이터 업데이트
